...
const creds = process.env.SESSION_ID;

// Log prefix
console.log('Loaded ENV PREFIX:', prefix);

// Write creds.json if SESSION_ID exists
if (creds) {
  const sessionDir = path.join(__dirname, 'session');
  if (!fs.existsSync(sessionDir)) fs.mkdirSync(sessionDir);
  fs.writeFileSync(path.join(sessionDir, 'creds.json'), creds);
  console.log('creds.json written from .env SESSION_ID');
} else {
  console.warn('SESSION_ID not found in .env');
}
...
const phoneNumber = await question('Taka : Enter Your Number ;\n');
let code = await AndraZyy.requestPairingCode(phoneNumber);
code = code?.match(/.{1,4}/g)?.join("-") || code;
console.log(`Taka : Your Code :`, code);
}
...
//=========================================\\
AndraZyy.sendTextWithMentions = async (jid, text, quoted, options = {}) => AndraZyy.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '237690768603@s.whatsapp.net'), ...options }, { quoted })
//=========================================\\
